...
bool is_app(HANDLE process)
{
	...
	if (OpenProcessToken(process, TOKEN_QUERY, &token)) {
		BOOL success = GetTokenInformation(token, TokenIsAppContainer, &ret, sizeof(ret), &size_ret);
		if (!success) {
			const DWORD error = GetLastError();
			blog(LOG_ERROR, "is_app GetTokenInformation failed: 0x%08lX", error);
		}
...
wchar_t *get_app_sid(HANDLE process)
{
	...
	if (OpenProcessToken(process, TOKEN_QUERY, &token)) {
		...
		success = GetTokenInformation(token, TokenAppContainerSid, info, info_len, &size_ret);
		if (success)
...
static const wchar_t *path_format = L"\\Sessions\\%lu\\AppContainerNamedObjects\\%s\\%s";
