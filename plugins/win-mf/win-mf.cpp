...
struct MediaFoundationSourceInput {
	obs_source_t *source;
	CAPTURE_DEVICE_HANDLE _mfcatpureDevice = nullptr;
...
void MediaFoundationSourceInput::MediaFoundationSourceLoop()
{
	while (true) {
		...
		case Action::Shutdown:
			if (_mfcatpureDevice) {
				MF_Stop(_mfcatpureDevice);
				MF_Destroy(_mfcatpureDevice);
				_mfcatpureDevice = nullptr;
			}
			return;
		...
		case Action::RestoreSettings:
			if (_mfcatpureDevice) {
				MF_RestoreDefaultSettings(_mfcatpureDevice);
			}
			break;
		case Action::NpuControl:
			if (_mfcatpureDevice) {
				...
				MF_GetBlur(_mfcatpureDevice, _blur,
					   _shallowfocus, _mask);
				MF_GetTransparent(_mfcatpureDevice,
						  _transparent);
				if (_blur != blur ||
				    _shallowfocus != shallowfocus ||
				    _transparent != mask) {
					MF_SetBlur(_mfcatpureDevice, blur,
						   shallowfocus, mask);
				}
				...
				bool _autoframing;
				MF_GetAutoFraming(_mfcatpureDevice,
						  _autoframing);
				if (_autoframing != autoframing) {
					MF_SetAutoFraming(_mfcatpureDevice,
							  autoframing);
				}
				...
				bool _eyegaze;
				MF_GetEyeGazeCorrection(_mfcatpureDevice,
							_eyegaze);
				if (_eyegaze != eyegaze) {
					MF_SetEyeGazeCorrection(
						_mfcatpureDevice, eyegaze);
				}
				obs_data_release(settings);
			}
			break;
...
bool MediaFoundationSourceInput::UpdateVideoConfig(obs_data_t *settings)
{
...
#if 0 // please delete this block after couple of validations on multiple systems. Iwamoto
	if (_mfcatpureDevice) {
		MF_Stop(_mfcatpureDevice);
		MF_Destroy(_mfcatpureDevice);
		_mfcatpureDevice = nullptr;
	...
	_videoConfig.cx = cx;
	_videoConfig.cy_abs = abs(cy);
	_videoConfig.cy_flip = cy < 0;
	_videoConfig.frameInterval = interval;

	_mfcatpureDevice = MF_Create(dev.path.c_str());
	if (_mfcatpureDevice) {
		HRESULT hr = MF_Prepare(_mfcatpureDevice);
		UINT32 w, h, fpsd, fpsn;
		if (SUCCEEDED(hr)) {
			if (resType != ResType_Custom) {
				hr = MF_GetOutputResolution(
					_mfcatpureDevice, &w, &h, &fpsn, &fpsd);
				if (SUCCEEDED(hr)) {
					interval = 10000000ll * fpsd / fpsn;
				}
			} else {
				w = _videoConfig.cx;
				h = _videoConfig.cy_abs;
				interval = _videoConfig.frameInterval;
			}
		}
		if (SUCCEEDED(hr)) {
			hr = MF_SetOutputResolution(
				_mfcatpureDevice, w, h, interval,
				MF_COLOR_FORMAT::MF_COLOR_FORMAT_ARGB);
			_videoConfig.cx = w;
			_videoConfig.cy_abs = h;
			_videoConfig.frameInterval = interval;
		}
		if (SUCCEEDED(hr)) {
			_firstframe = true;
			hr = MF_Start(_mfcatpureDevice, _OnVideoData, this);
		}
