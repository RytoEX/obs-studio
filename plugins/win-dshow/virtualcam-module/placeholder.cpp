...
static std::vector<std::wstring> get_placeholder_paths()
{
	...
	// file #1: "custom-placeholder.png" next to loaded DLL
	StringCbCopy(new_file, sizeof(new_file), dll_path);
	StringCbCat(new_file, sizeof(new_file), L"custom-placeholder.png");
	paths.emplace_back(std::wstring(new_file));
	StringCbCopy(new_file, sizeof(new_file), dll_path);

	// file #2: "custom-placeholder.png" in AppData plugin_config
	PWSTR pszPath = NULL;
	...
	// file #3: the default "placeholder.png" next to loaded DLL
